...
def download_dailymotion_clip():
    ...
    try:
        ...
        ydl_opts = {
            'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best', # Prioritize MP4 video and audio, fallback to best
            'outtmpl': output_path,
            'verbose': True,
            'no_warnings': False,
            'ignoreerrors': True,
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                try:
                    ...
                    if not info:
                        print(f"Debug - Could not extract video info for {video_id}")
                        return jsonify({'success': False, 'error': 'Video not found or unavailable'})
                    print(f"Debug - Successfully extracted video info for {video_id}")
                except Exception as e:
                    print(f"Debug - Error extracting video info for {video_id}: {str(e)}")
                    return jsonify({'success': False, 'error': f'Error accessing video: {str(e)}'})
                ...
                try:
                    ...
                    print(f"Debug - Error during download for {video_id}: {str(e)}")
                    if os.path.exists(output_path):
                        try:
                            os.remove(output_path)
                        except: # nosec
            ...
            print(f"Successfully downloaded Dailymotion clip to {output_path}")
            return jsonify({
                'success': True,
                'filename': output_filename
            })
        ...
        print(f"Debug - General error in download_dailymotion_clip for {video_id}: {str(e)}")
        return jsonify({'success': False, 'error': f'Unexpected error: {str(e)}'})

@app.route('/add-dailymotion-to-video', methods=['POST'])
@login_required
def add_dailymotion_to_video():
    """Add a Dailymotion clip to the user's video by downloading and merging."""
    videos = []
    input_paths = []
    final_video = None
    dailymotion_path = None # Define dailymotion_path outside try for cleanup
    ...
    try:
        ...
        if not video_id:
            return jsonify({'success': False, 'error': 'No Dailymotion video ID provided'})
            
        if not video_file.filename:
            return jsonify({'success': False, 'error': 'No selected file'})
            
        print(f"Debug - Processing merge request for Dailymotion video ID: {video_id}")
        ...
        # Download the Dailymotion clip using yt-dlp
        dailymotion_filename = f'dailymotion_{video_id}_{int(time.time())}.mp4'
        dailymotion_path = os.path.join(app.config['OUTPUT_FOLDER'], dailymotion_filename)
        input_paths.append(dailymotion_path) # Add to input_paths for cleanup
        
        print(f"Debug - Downloading Dailymotion video for merge using yt-dlp to: {dailymotion_path}")
        
        # Configure yt-dlp options for Dailymotion merge
        ydl_opts = {
            'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best', # Prioritize MP4 video and audio, fallback to best
            'outtmpl': dailymotion_path,
            'verbose': True,
            'no_warnings': False,
            'ignoreerrors': True,
            'quiet': False,
        ...
        try:
            print(f"Debug - Starting yt-dlp download for merge to: {dailymotion_path}")
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                 # First try to extract info to verify the video exists and is downloadable
                try:
                    info = ydl.extract_info(f'https://www.dailymotion.com/video/{video_id}', download=False)
                    if not info:
                        print(f"Debug - Could not extract video info for merge for {video_id}")
                        cleanup_files(input_paths)
                        return jsonify({'success': False, 'error': 'Video not found or unavailable for merge.'})
                    print(f"Debug - Successfully extracted video info for merge for {video_id}")
                except Exception as e:
                    print(f"Debug - Error extracting video info for merge for {video_id}: {str(e)}")
                    cleanup_files(input_paths)
                    return jsonify({'success': False, 'error': f'Error accessing video for merge: {str(e)}'})

                # Now try to download the video
                try:
                    print(f"Debug - Initiating download for merge for {video_id}")
                    ydl.download([f'https://www.dailymotion.com/video/{video_id}'])
                    print(f"Debug - yt-dlp download process finished for merge for {video_id}")
                except Exception as e:
                    print(f"Debug - Error during download for merge for {video_id}: {str(e)}")
                    cleanup_files(input_paths)
                    return jsonify({'success': False, 'error': f'Error downloading video for merge: {str(e)}'})

            # Verify the downloaded file
            if not os.path.exists(dailymotion_path):
                print(f"Debug - Output file not found after download for merge at {dailymotion_path}")
                cleanup_files(input_paths)
                return jsonify({'success': False, 'error': 'Failed to download video for merge: Output file not created.'})

            if os.path.getsize(dailymotion_path) == 0:
                print(f"Debug - Downloaded file for merge is empty at {dailymotion_path}")
                cleanup_files(input_paths)
                return jsonify({'success': False, 'error': 'Downloaded video file for merge is empty.'})

            print(f"Debug - Successfully downloaded video for merge to: {dailymotion_path}")

            print(f"Debug - Loading videos for merging")
            # Load the uploaded video
            video1 = mp.VideoFileClip(video_path)
            # Load the downloaded Dailymotion clip
            video2 = mp.VideoFileClip(dailymotion_path)
            videos.extend([video1, video2]) # Add to videos list for cleanup
            
            # Get the original dimensions of the first video
            target_width = video1.w
            target_height = video1.h
            
            print(f"Debug - Resizing Dailymotion video to match original video size: {target_width}x{target_height}")
            
            # Resize the second video to match the first video's dimensions
            if video2.w != target_width or video2.h != target_height:
                video2 = video2.resize(width=target_width, height=target_height)
            ...
            output_filename = f'merged_dailymotion_{int(time.time())}.mp4'
            output_path = os.path.join(app.config['OUTPUT_FOLDER'], output_filename)
            
            print(f"Debug - Writing final merged video to {output_path}")
            print(f"Debug - Starting write_videofile with preset 'fast' and progress logger for Dailymotion merge")
            
            # Merge the videos using ffmpeg
            try:
                # Use MoviePy's concatenate_videoclips for merging
                ...
                final_video.write_videofile(
                    ...
                    bitrate='8000k',
                    fps=30,
                    preset='fast',  # Use fast preset for faster processing
                    threads=4,
                    ffmpeg_params=[
                        '-crf', '18',
                        '-profile:v', 'high',
                ...
                print(f"Debug - Finished write_videofile for Dailymotion merge")
                
                print(f"Debug - Cleaning up video objects and files after Dailymotion merge")
                ...
                print(f"Debug - Dailymotion merge completed successfully")
                return jsonify({
                    'success': True,
                    'output_file': output_filename
                })
                ...
                print(f"Debug - FFmpeg processing error during Dailymotion merge: {str(e)}")
                # Clean up on error
                cleanup_videos(videos)
                if final_video:
                    final_video.close()
            ...
            print(f"Debug - yt-dlp execution error for merge for {video_id}: {str(e)}")
            cleanup_files(input_paths)
            return jsonify({'success': False, 'error': f'Error processing download for merge: {str(e)}'})
